...
- **ファイルアップロード API**: base64 形式で画像・テキスト・PDF・Office文書・CSV をアップロードしてセッションで利用
- **プロジェクトファイル閲覧 API**: プロジェクト内のテキスト・画像ファイルの内容を取得（pipe-viewerのようなビューワーUI向け）。パストラバーサル・隠しファイル・`.gitignore`対策を内蔵
