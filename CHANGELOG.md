...
### Added
- **Automatic cleanup of old `/attachments` files**: files in `/tmp/claude-code-pipe/` older than `config.upload.maxAgeDays` (default `7` days) are now deleted on server start and every hour thereafter, based on file mtime
